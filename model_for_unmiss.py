...
def split_dev_on_downsample(dev_idxes, labels, fold=5):
        ...
        for para_est in para_ests:
            for para_dep in para_deps:
                for para_leaf in para_leafs:
                    ...
                    train_acc, train_auc, train_f1 = utils.get_matrix(train_pred_y, train_y)
                    val_acc, val_auc, val_f1 = utils.get_matrix(val_pred_y, val_y)
                    test_acc, test_auc, test_f1 = utils.get_matrix(test_pred_y, test_y)
                    ext_acc, ext_auc, ext_f1 = utils.get_matrix(ext_pred_y, ext_y)
                    ...
                    print(f'train {iteration} perf: {train_acc, train_auc, train_f1}; paras: {para_est, para_dep, para_leaf}')
                    print(f'val {iteration} perf: {val_acc, val_auc, val_f1}; paras: {para_est, para_dep, para_leaf}')
                    print(f'test {iteration} perf: {test_acc, test_auc, test_f1}; paras: {para_est, para_dep, para_leaf}')
                    print(f'external {iteration} perf: {ext_acc, ext_auc, ext_f1}; paras: {para_est, para_dep, para_leaf}')
            ...
            for para_dep in para_deps:
                for para_leaf in para_leafs:
                    ...
                    if (
                        ...
                        print(f'train {iteration} perf: {train_acc, train_auc, train_f1}; paras: {para_est, para_dep, para_leaf}')
                        print(f'val {iteration} perf: {val_acc, val_auc, val_f1}; paras: {para_est, para_dep, para_leaf}')
                        print(f'test {iteration} perf: {ext_acc, ext_auc, ext_f1}; paras: {para_est, para_dep, para_leaf}')
                        print(f'external {iteration} perf: {ext_auc, ext_acc}; paras: {para_est, para_dep, para_leaf}')
                        train_pids = pids[0]
                        val_pids = pids[1]
                        test_pids = pids[2]
                        ext_pids = pids[3]
...
if __name__ == "__main__":
    ...
    # modes = ["bayes", "decision_tree", "random_forest", "neutral_network", "xgboost", "logistic", "svm"]
    modes = ["svm"]
    for mode in modes:
        ...
        for i, ll in enumerate(fold_idxes_lst):
            ...
            if mode in ["bayes", "svm", "decision_tree", "random_forest", "neutral_network", "xgboost", "logistic"]:
                cur_outdir = os.path.join(tmp_outdir, mode+'8')
                os.makedirs(cur_outdir, exist_ok=True)
                ...
                print(f"For {mode} and ext, ext acc: {ext_acc:.4f}, ext auc: {ext_auc:.4f}, ext_f1: {ext_f1: .4f}, {ext_sens, ext_spec, ext_ppv, ext_npv, ext_auprc}")
