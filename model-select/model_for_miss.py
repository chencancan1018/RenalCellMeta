    ...
    for i, ll in enumerate(fold_idxes_lst):
        ...
        for ms in model_strs:
            ...
            if ms == "svm":
                ...
                model = RandomForestClassifier(n_estimators=30, max_depth=10, min_samples_leaf=5, bootstrap=True)
            elif ms == "decision_tree":
                model = DecisionTreeClassifier(max_depth=20, min_samples_split=10, min_samples_leaf=10)
            ...
            print(f"For ext dataset, ext acc: {ext_acc:.4f}, ext auc: {ext_auc:.4f}, ext_f1: {test_f1: .4f}, {ext_sens, ext_spec, ext_ppv, ext_npv, ext_auprc}")
            end_time = time.time()
